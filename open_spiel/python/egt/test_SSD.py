...
from open_spiel.python.egt.kosaraju import *
import sympy as sp
from sympy import *
...
def reduce(mat, s, M0 = None):
    ...
    dim = mat.shape[1]
    if M0 is None:
        # Support scipy sparse numeric matrices by converting to dense numeric
        # arrays. Otherwise evaluate polynomial entries at epsilon=0.
        if sps.issparse(mat):
            M0 = np.array(mat.toarray(), dtype=float)
        else:
...
def SSD_step(mat):
    ...
    result = {}
    # Support sparse numeric matrices as input by materializing M0 accordingly.
    if sps.issparse(mat):
        M0 = mat
    else:
        M0 = np.array(evalPolyArray(mat,0), dtype=float)
...
def SSD_iterate_optimized(mat):
    ...
    while True:
        result = SSD_step(mat)
        mat = copy.deepcopy(result['mat'])
        if 'stab' in result:
            break
        elif 'i' in result:
            if left_mults:
                left_mults = left_mults @ result['i']
            else:
                left_mults = copy.deepcopy(result['i'])
        else:
            if left_mults:
                left_mults = left_mults @ result['D']
            else:
                left_mults = copy.deepcopy(result['D'])

    if left_mults is None:
        return result['stab']
    return left_mults @ result['stab']

def SSD(mat):
    """Computes the SSD of a PMM."""
    # If mat is a sparse numeric matrix, convert for graph computations.
    mat_for_graph = mat
    # if sps.issparse(mat):
    #     mat_for_graph = np.array(mat.toarray(), dtype=float)

    g = Graph(mat_for_graph, polyZeroTest)
    if len(g.CommunicatingClasses().keys()) > 1:
        raise Exception("Input must be unichain.")

    stab = SSD_iterate(mat)

    if sps.issparse(stab):
        vec = np.array(stab.toarray(), dtype=float).flatten()
    else:
        vec = np.array(evalPolyArray(stab,0), dtype=float)
...
def construct_perturbed_markov_matrix_hpt(payoff_matrices, m, s):
    """Construct the perturbed Markov matrix for adaptive learning dynamics.
    Supports >2 players.
    :param payoff_matrices: list of hpt payoff tables
    :param m: finite history length (m = 4 -> dqqdDQDQ)
    :param s: limited sample of opponent's moves (m = 4, s = 2 -> qqDQDQ, dqDQDQ, ...)
    """
    games = payoff_matrices
    num_players = len(games)
    n_actions = (games[0]).shape[0]
    n_states = (n_actions ** m) * ((n_actions ** s) ** (num_players-1))
