...
class Graph(object):
    ...
    def CommunicatingClasses(self):
        ...
        # Explore the graph to order the vertices according
        # to the "can reach" pre-order.
        for vertex in range(self.dim):
            self.Visit(vertex)
        
        # Do a second search on the reverse graph to assign
        # to communicating classes, i.e., strongly connected components (scc).
        for vertex in self.sortedVertices:
            self.Assign(vertex, vertex)

        # Convert the components dictionary from a mapping from
        # all vertices to their scc representative, to a dictionary
        # mapping the representatives to is scc members.
    ...
    def ClosedClasses(self):
        ...
        # Make sure that communicatingClasses has been initialized
            ...
            for source in self.communicatingClasses[cc]:
                if self.is_sparse:
                    ...
                    for target in range(self.dim):
                        # If there is an edge from outside into the class member
                    ...
                    # No need to continue checking further, since cc is not closed
